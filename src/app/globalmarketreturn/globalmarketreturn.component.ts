...
export class GlobalMarketReturnComponent implements OnInit {
  ...
  getMcsiAcwiData(): void {
    ...
    const startIndex = msciAcwiIndex.findIndex(msciAcwi => new Date(msciAcwi.Date).getTime() === startDate.getTime());

    const endDate = this.aggregatedCreditData.mortageIntervals[0].endDate;
    const endIndex = msciAcwiIndex.findIndex(msciAcwi => new Date(msciAcwi.Date).getTime() === endDate.getTime());
    ...
    const monthlyInterestRate = this.aggregatedCreditData.mortageIntervals[0].annualPercentageRate / 100 / 12;
    const paymentsNumberInMonth = 12 * (endDate.getFullYear() - startDate.getFullYear()) + endDate.getMonth() - startDate.getMonth() + 1; // +1 cause we need to pay for the first month too
    const monthlyInterest = loanAmount *
      (monthlyInterestRate * Math.pow(1 + monthlyInterestRate, paymentsNumberInMonth)) / (Math.pow(1 + monthlyInterestRate, paymentsNumberInMonth) - 1);

    const initialCost = this.aggregatedCreditData.mortageIntervals[0].closingCost + this.aggregatedCreditData.downPayment;
    var capitalGainNominal = initialCost + monthlyInterest; // therefore i = startIndex + 1 we start from index + 1, however we have a still initial payment
    var capitalGainReal = capitalGainNominal;
    ...
    var monthlyPaymentsReal = monthlyInterest; // in the very first month we do not have inflation MoM, therefore i = startIndex + 1 we start from index + 1, however we have a still initial payment
    var totalInitialPaymentReal = initialCost;
    ...
    for (let i = startIndex + 1; i <= endIndex; i++) {
      ...
      monthlyPaymentsReal = monthlyPaymentsReal * (1 - vpiInflationMonthly[i]!.InflationMoM! / 100) + monthlyInterest;
      totalInitialPaymentReal = totalInitialPaymentReal * (1 - vpiInflationMonthly[i]!.InflationMoM! / 100);
  ...
  private calculateAnnualizedReturn(capitalGain: number, investedAmount: number, duration: number) {
    // annualized return, also known as the compound annual growth rate
    // CAGR = ( Final Value / Initial Investment ​) ^ (1 / Number of Years) − 1
    var returnPerAnnum = (Math.pow(capitalGain / investedAmount, 1 / duration) - 1) * 100;
